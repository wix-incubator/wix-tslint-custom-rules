...
describe('noUntypedPublicSignatureRule', () => {
  describe('default options', () => {
    it('should not allow :any in public signature', () => {
      ...
      expect(result.failures[0].getFailure()).toBe(
        'All arguments of public method must have types.',
      );
    });
    ...
    it('should raise two errors when method has untyped parameter and untyped return value', () => {
      ...
      expect(result.failures[1].getFailure()).toBe(
        'Public methods must have return type.',
      );
    });
  ...
  describe('with "any-allowed" argument', () => {
    it('should allow :any return type', () => {
      ...
      const result = helper({
        src,
        rule: { name: rule, options: 'allow-any' },
      });
      expect(result.errorCount).toBe(0);
    });
  ...
  describe('with allowed methods', () => {
    it('should allow untyped public method', () => {
      ...
      const result = helper({
        ...
        rule: {
          name: rule,
          options: [{ allowedMethods: ['componentWillMount', 'render'] }],
        },
      });
      expect(result.errorCount).toBe(0);
    });
    ...
    it('should support multiple allowedMethods options', () => {
      ...
      const result = helper({
        ...
        rule: {
          name: rule,
          options: [
            { allowedMethods: ['componentWillMount'] },
            { allowedMethods: ['render'] },
          ],
        },
      });
      expect(result.errorCount).toBe(0);
