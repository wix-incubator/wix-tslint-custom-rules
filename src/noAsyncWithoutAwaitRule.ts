...
class Walk extends Lint.RuleWalker {
  ...
  private functionBlockHasAwait(node: ts.Node): boolean {
    ...
    const awaitInChildren = node
      .getChildren()
      .map((functionBlockNode: ts.Node) =>
        this.functionBlockHasAwait(functionBlockNode),
      );
  ...
  private functionBlockHasReturn(node: ts.Node): boolean {
    ...
    const returnInChildren = node
      .getChildren()
      .map((functionBlockNode: ts.Node) =>
        this.functionBlockHasReturn(functionBlockNode),
      );
    return returnInChildren.some(Boolean);
  }

  private isShortArrowReturn(
  ...
  private addFailureIfAsyncFunctionHasNoAwait(
    node: ts.ArrowFunction | ts.FunctionDeclaration | ts.MethodDeclaration,
  ) {
    if (
